...
export class CodeVerifyComponent {
  ...
  onResendCode(): void {
    this.registerService.resendVerificationCode(this.userId).subscribe({
      next: (response) => {
        Swal.fire('Éxito', response, 'success'); // Muestra un mensaje de éxito
      },
      error: (err) => {
        console.error('Error al reenviar el código: ', err);
        const errorMessage = err.error?.message || err.message;
        Swal.fire('Error', 'No se pudo reenviar el código. Detalles: ' + errorMessage, 'error');
      }
    });
  }
}